...
# ====================== КЛАСС БАЗЫ ДАННЫХ ======================
class DeliveryDB:
    def __init__(self, db_path: str = "zakazi.db"):
        self.db_path = db_path
        self.init_db()

    def init_db(self):
        """Создание базы данных с восстановлением при повреждении"""
        conn = None
        try:
            # Пробуем подключиться и проверить целостность
            ...
            if result != "ok":
                logger.warning(f"⚠️ База повреждена: {result}. Пересоздаём...")
                conn.close()
                import os
            ...
            logger.warning(f"⚠️ Ошибка проверки БД: {e}. Пересоздаём...")
            if conn:
                conn.close()
            import os
            if os.path.exists(self.db_path):
        ...
        # ИСПРАВЛЕННЫЕ индексы (была ошибка в именах колонок!)
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_company ON deliveries(company)",  # ← Было company_name!
        ...
        for idx_sql in indexes:
            try:
                ...
                logger.warning(f"⚠️ Пропуск индекса: {e}")
...
# Включение CORS для работы с Android приложением
app.add_middleware(
    CORSMiddleware,
...
def get_zone_diff(city1: str, city2: str) -> int:
    """Получить разницу зон между городами"""
...
def get_offers(weight: float, box_size: BoxSize, zone_diff: int) -> List[DeliveryOffer]:
    """Получить список предложений по доставке"""
...
def generate_tariffs_for_city(city: str, weight: float) -> List[TariffItem]:
    """Генерация тарифов для Android приложения"""
    tariffs = []
    ...
    for i, company in enumerate(companies):
        ...
        # Определяем, восстановлены ли данные
...
def calculate_and_save_delivery(input_string: str) -> str:
    """
    🎯 ИЗ СТРОКИ в БД и обратно в строку!
    Формат: "Москва Санкт-Петербург 2.5 M"
    """
    try:
        # 1. Парсинг строки
        ...
        best_offer = offers[0]  # Самый дешёвый
...
async def get_tariffs(request: TariffsRequest):
    """
    Основной эндпоинт для Android приложения
    """
    ...
    # ✅ СОХРАНЕНИЕ ЛУЧШЕГО ТАРИФА В БД
    if tariffs:
        ...
        try:
            delivery_id = db.save_delivery(
                ...
                size="M",  # По умолчанию
                town_from="Москва",  # Отправление всегда из Москвы по ТЗ
...
async def calculate_delivery(
    ...
    """Расчет доставки с сохранением в БД (для веб-интерфейса)"""
    ...
    # Проверка веса для выбранного размера
    if weight > BOX_DIMENSIONS[box_size]["max_weight"]:
        raise HTTPException(
            status_code=400,
    ...
    # ✅ СОХРАНЕНИЕ ВСЕХ ПРЕДЛОЖЕНИЙ В БД
    for offer in offers[:3]:  # Сохраняем топ-3
...
# ====================== ЭНДПОИНТЫ ДЛЯ УПРАВЛЕНИЯ БД ======================
@app.get("/api/deliveries")
async def get_all_deliveries():
    """Получить все сохранённые доставки"""
...
async def get_statistics():
    """Статистика по сохранённым доставкам"""
    deliveries = db.get_all_deliveries()
        ...
        <div class="grid md:grid-cols-2 gap-8 mb-12">
            ...
            <div class="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/50 card-hover">
                <h2 class="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                    <span class="mr-2">📦</span> Рассчитать доставку
                </h2>
...
?from_city=Москва&to_city=Казань&weight=3.5&box_size=M
    ...
    <script>
        ...
        async function clearDatabase() {{
            if (confirm('🗑️ Вы уверены, что хотите очистить всю базу данных? Это действие нельзя отменить.')) {{
        ...
        // Автоматическое заполнение примером
